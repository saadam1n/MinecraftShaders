...
float ComputeSSAO(in SurfaceStruct surface){
    ...
    vec3 Tangent = cross(surface.ViewNormal, normalize(texture2D(noisetex, surface.Screen.xy + frameTimeCounter).rgb));
    ...
    for(int sample = 0; sample < SSAO_SAMPLES; sample++){
        vec3 RandomDirection = vec3 (
            texture2D(noisetex, vec2(0.1f + (sample * 1.0f) / 64.0f)).rgb // Taken from contiuum 1.3
        );
        RandomDirection.xy = RandomDirection.xy * 2.0f - 1.0f;
