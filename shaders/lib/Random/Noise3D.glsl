...
vec2 GenerateNoise3D_4(vec3 P, float jitter = 1.0f, bool manhattanDistance = false) {
...
/* 3d simplex noise */
float GenerateNoise3D_5(vec3 p) {
...
float GenerateNoise3D_6(vec3 m) {
    return   0.5333333*GenerateNoise3D_5(m*rot1)
			+0.2666667*GenerateNoise3D_5(2.0*m*rot2)
			+0.1333333*GenerateNoise3D_5(4.0*m*rot3)
			+0.0666667*GenerateNoise3D_5(8.0*m);
}
...
float GenerateNoise3D_7(vec3 P){
...
float GenerateNoise3D_8(vec3 v){ 
...
float GenerateNoise3D_10(vec3 uv, float persistence = 0.7f, int octaves = 8){
	float XY = GenerateNoise2D_3(uv.xy, persistence, octaves); 
	float XZ = GenerateNoise2D_3(uv.xz, persistence, octaves); 
	float YZ = GenerateNoise2D_3(uv.yz, persistence, octaves);
...
#define NUM_CELLS	10.0	// Needs to be a multiple of TILES!
#define TILES 		2.0		// Normally set to 1.0 for a creating a tileable texture.

//-----------------------------------------------------------------------------
// Quick 'n Dirty HLSL to GLSL
//-----------------------------------------------------------------------------
#define float2 vec2
#define float3 vec3
#define float4 vec4
...
float noise(in float3 x)
{
	...
	return lerp(
			lerp(
				lerp(hash3d1(n + 0.0), hash3d1(n + 1.0), f.x),
				lerp(hash3d1(n + 57.0), hash3d1(n + 58.0), f.x),
				f.y),
			lerp(
				lerp(hash3d1(n + 113.0), hash3d1(n + 114.0), f.x),
				lerp(hash3d1(n + 170.0), hash3d1(n + 171.0), f.x),
				f.y),
			f.z);
}
...
float GenerateNoise3D_11(vec3 p){
	float c0 = Cells(p, NUM_CELLS);
	float c1 = Cells(p * 2.0, NUM_CELLS);
	float c2 = Cells(p * 4.0, NUM_CELLS);
	float c3 = Cells(p * 8.0, NUM_CELLS);
    return (c0 * ( ( 1.0 - c1) + c2 + c3) );
}
...
float GenerateNoise3D_12(vec3 p) {
    ...
    col.b += GenerateNoise3D_9(p, freq*2.);
    col.a += GenerateNoise3D_9(p, freq*4.);
